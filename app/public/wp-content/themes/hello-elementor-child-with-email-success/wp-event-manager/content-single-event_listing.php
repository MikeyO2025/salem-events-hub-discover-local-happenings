...
$location = urlencode(get_post_meta(get_the_ID(), '_event_location', true));
$description = urlencode(strip_tags(get_post_meta(get_the_ID(), '_event_description', true)));
$start = date("Ymd\THis", strtotime(get_post_meta(get_the_ID(), '_event_start_date', true)));
$end = date("Ymd\THis", strtotime(get_post_meta(get_the_ID(), '_event_end_date', true)));
$link = "https://calendar.google.com/calendar/render?action=TEMPLATE&text={$title}&dates={$start}/{$end}&details={$description}&location={$location}";
?>
<div class="event-google-calendar" style="margin: 20px 0;">
    <a href="<?php echo esc_url($link); ?>" target="_blank" rel="noopener noreferrer"
       class="google-calendar-button"
       style="display: inline-block; padding: 10px 15px; background-color: #4285F4; color: #fff; border-radius: 5px; text-decoration: none; font-weight: bold;">
        📅 Add to Google Calendar
    </a>
</div>
<!-- End Google Calendar Button -->
